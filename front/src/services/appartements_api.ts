...
export async function createAppartement(data: Omit<Appartement, 'numApp'>): Promise<Appartement> {
    try {
        const response = await axios.post<Appartement>(`${API_BASE_URL}/appartement`, data);
...
export async function updateAppartement(data: Appartement): Promise<void> {
    try {
        await axios.put(`${API_BASE_URL}/appartement/${data.numApp}`, data);
    } catch (error) {
        if (axios.isAxiosError(error)) {
...
export async function getAllAppartement(): Promise<Appartement[]> {
    try {
        const response = await axios.get<Appartement[]>(`${API_BASE_URL}/appartement`);
        return response.data;
    } catch (error) {
        if (axios.isAxiosError(error)) {
            throw new Error(error.response?.data?.error || "Erreur lors de la récupération");
